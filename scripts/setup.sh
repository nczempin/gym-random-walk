...
if command_exists python3.7; then
    ...
    echo -e "${GREEN}Found Python 3.7${NC}"
    ...
    if [[ $PYTHON_VERSION == 3.7.* ]]; then
        ...
        echo -e "${YELLOW}This project works best with Python 3.7 for gym 0.7.4 compatibility${NC}"
        ...
        if [ "$IS_DEBIAN_BASED" = true ]; then
            ...
            echo -e "${RED}Please install Python 3.7 manually and run this script again.${NC}"
            echo -e "${RED}This project requires Python 3.7 for compatibility with gym 0.7.4${NC}"
            exit 1
        fi
    fi
else
...
# Check for pip for Python 3.7
if ! $PYTHON_CMD -m pip --version >/dev/null 2>&1; then
    echo "pip for Python 3.7 is not installed."
    if [ "$IS_DEBIAN_BASED" = true ]; then
        ...
        echo -e "${RED}Please install pip for Python 3.7 manually and run this script again.${NC}"
        exit 1
    fi
fi

# Remove existing venv if it exists (to ensure we use Python 3.7)
if [ -d "venv" ]; then
    echo -e "${YELLOW}Removing existing virtual environment to recreate with Python 3.7...${NC}"
    rm -rf venv
fi
